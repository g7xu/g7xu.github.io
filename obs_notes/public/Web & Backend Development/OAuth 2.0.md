**Summary**: The OAuth 2.0 authorization framework — roles, tokens, grant types, the authorization-code flow, OIDC, and Spring Security integration.

**Last updated**: 2026-05-29

---

OAuth 2.0, also called open authorization, is an industry-standard protocol for secure authorization (not authentication — that is handled by [[#OIDC]], an identity layer built on top of it). The protocol enables third party apps to access user data without sharing their credentials, such as username and passwords. Instead the protocol let's the client(third party app) access or query information using access tokens (generated by OAuth 2.0 provider).
