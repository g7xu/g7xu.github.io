...
- Compute + Storage
- Compute = CPU + RAM
- Storage = Hard Disk (Block storage)
- Network Interface

S3 - Simple Storage Service
- Object Based Storage (file storage, media storage)
- Not OS or DB storage
- File up to 5 TB
- Unlimited Storage (if you pay)
- Bucket
	- name must be globally unique across all AWS users
	- when bucket created, go with a region which should be close to users to reduce latency
		- can't change upon creation
	- 100 buckets per account by default
	- Object in bucket
		- Key Value Pair
		- S3 Versioning is enabled, then if the same object is pushed to the bucket again, the new object will get a unique Version ID
		- 
## Why
- Cost Saving - pay as you go
- Fast start - Boot new server instances in min
- Easy to scale up/down
## Pricing Options
- On Demand -- fixed rate by hour
- Reserved -- Capacity Reservation with discount
- Spot -- Big whatever the price you want for instance capacity
- Dedicated Hosts -- Enable you to use your eligible software licenses from vendors
## [EC2 Families](https://www.geeksforgeeks.org/devops/amazon-ec2-instance-types/): Handle different workflows
 - T: Cheap General Purpose
 - M: General Purpose
## EC2 Storage
- Instance Store
	- Ephemeral Storage -- goes away after instance got terminated
- Elastic Block Storage (EBS)
	- Highly available and scalable storage volumes 
## Network Interface
the point of interconnection between a computer/server and a private or public network

**AWS ENI** (Elastic Network Interface) -- virtual network card that providing:
- IP Address: private and public
- Security Groups: which port is exposed and which is not
	- All Inbound traffic is blocked by default
	- All Outbound traffic is allowed
	- STATEFUL -- when inbound rule allowing traffic in, that traffic is automatically allowed back out
	- change to security group take effect immediately
	- each security group can be attach to different resources
- Mac Address

## Usual Practice
- Start our EC2 instance
